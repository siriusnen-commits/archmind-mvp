...
def test_ui_runtime_action_run_all_does_not_stop_on_intermediate_frontend_not_running(monkeypatch, tmp_path: Path) -> None:
    projects_root = tmp_path / "projects"
    _make_project(projects_root, "runtime-poll-intermediate-not-running")
    monkeypatch.setenv("ARCHMIND_PROJECTS_DIR", str(projects_root))
    ...
    runtime_sequence = [
        ...
        {
            "backend": {"status": "RUNNING", "url": "http://127.0.0.1:8000"},
            "frontend": {"status": "NOT RUNNING", "url": "http://127.0.0.1:3000"},
        },
        {
            "backend": {"status": "RUNNING", "url": "http://127.0.0.1:8000"},
            "frontend": {"status": "RUNNING", "url": "http://127.0.0.1:3000"},
    ...
    client = TestClient(create_ui_app())
    response = client.post("/ui/projects/runtime-poll-intermediate-not-running/run-all")
    assert response.status_code == 200
    payload = response.json()
    assert payload["ok"] is True
    assert payload["backend_status"] == "RUNNING"
...
def test_ui_runtime_detail_expands_frontend_urls_from_verified_backend_hosts(monkeypatch, tmp_path: Path) -> None:
    projects_root = tmp_path / "projects"
    _make_project(projects_root, "runtime-final-serialization")
    monkeypatch.setenv("ARCHMIND_PROJECTS_DIR", str(projects_root))
    monkeypatch.setattr(
        "archmind.project_query.get_local_runtime_status",
        lambda _project_dir: {
    ...
    monkeypatch.setattr(
        ...
        lambda url, timeout_s=0.35: bool(
            str(url).startswith("http://192.168.0.197:3123") or str(url).startswith("http://100.117.128.20:3123")
        ),
    )

    client = TestClient(create_ui_app())
    response = client.get("/ui/projects/runtime-final-serialization")
    assert response.status_code == 200
    runtime = response.json()["runtime"]
    assert runtime["backend_urls"] == [
        "http://127.0.0.1:8123",
